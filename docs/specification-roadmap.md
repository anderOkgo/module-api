...
**Not every test layer can carry that acceptance-criteria weight equally** (clarified 2026-07-15, see Phase 7): unit and integration tests import this codebase's actual TypeScript classes, so they're inseparable from *this* implementation — they'd have to be rewritten from scratch in a rewrite-in-another-language scenario, which means they can't themselves be "the spec" for that scenario. The one layer that treats the system as a pure black box — real HTTP requests over the network to a running server, asserting only on wire-level responses and observable side effects, with zero source-code imports — is the portable one: point the exact same suite at *any* implementation (this one, or a from-scratch rewrite in another language) and it either passes or it doesn't. That's `scripts/smoke-test.js` (`npm run smoke`) — deliberately kept as a single plain script, not a test-framework project, precisely so it stays that simple/portable (see Phase 7's "false start" note on why a Jest-based rewrite of it was tried and reverted the same day). See Phase 7 for what that means concretely and what's still needed to make it a genuinely complete contract rather than a smoke-level subset.
...
**Update (2026-07-19, Phase 9)**: the audit-and-CI-gate half of this is now done — all three sibling repos got the same kind of schema-drift sweep and CI gate `module-api` itself has (see Phase 9 below), including real, previously-undetected drift between each repo's bootstrap and its actual production schema. Whether they should stay three separate git repos at all, versus a monorepo, is a separate, still-open question raised in the same pass — see Phase 9's closing section. Not decided, not implemented.
...
**Status: DONE** (all 4 items closed — see Phase 8, 4a/4b/4e, and 8b/8c)

1. **`docs/SPECIFICATION.md`** — generative design rules (not descriptive): module layering invariants, CQRS conventions, port/adapter rules, entity invariants, error-handling conventions. This is the "philosophy" layer an AI needs to regenerate the system rather than copy it. **Done, see Phase 8b.**
...
4. Consider ADRs for major architectural decisions (hexagonal layering, CQRS, eventual microservice split) and, if/when the module split happens, contract testing (e.g. Pact) between `auth`/`finan`/`series`. **Done, see Phase 8c** — the ADR part is written; the contract-testing sub-item stays explicitly deferred (no microservice split has happened, so there's nothing to write a contract-testing decision about yet — see `docs/adr/README.md`'s "Deferred" section).
...
**Status: DONE** (2026-07-14) — extended 2026-07-15 with one more check (see Phase 7): the script described below is still exactly what runs today via `npm run smoke`, just with a username-casing regression check added to it directly (Phase 7 also documents a same-day attempt to rewrite this whole script as a Jest suite, which was reverted — the user wanted this layer to stay a single simple file, not a test-framework project).
...
**Status: DONE** (closed 2026-07-18) — the reframing itself, the false-start-and-revert, and the concrete regression-check addition were finished 2026-07-15; closed as a phase once its two remaining "Still TODO" items resolved: item 2 (`docs/SPECIFICATION.md`) shipped in Phase 8b, and item 1 (expanding `scripts/smoke-test.js` toward exhaustiveness) was reframed as the ongoing practice it always actually was, not a task with a finish line — see "Ongoing, not a TODO" below.
...
| **`scripts/smoke-test.js`** | Plain Node `axios` HTTP requests to a **URL** (`http://localhost:3001` or any `BASE_URL`) of an **already-running** server — zero imports from `src/`, zero test-framework dependency at all | **Yes.** It only knows the wire contract: routes, request shapes, status codes, response JSON, and whatever it can observe through *other* endpoints. Point it at a Python/Go/Rust rewrite serving the same routes on the same port and it either passes or it doesn't, unmodified. |
...
### False start: converting it to a Jest suite (reverted same day)

First attempt at "Still TODO" item 1 below was a full rewrite into `test/acceptance/` — six TypeScript files (`helpers/client.ts`, `helpers/auth.ts`, one `*.acceptance.test.ts` per module), wired into `jest.config.js`/`package.json`, `describe.skip`-gated per credential tier. It worked (verified 36 passed/1 skipped against the real DB), but the user pushed back hard on seeing it: the actual ask was "un JS, uno solo" (one JS file, singular) — the whole point of this layer being *simple and portable* was undermined by making it depend on Jest/TypeScript/a 6-file directory structure to even run. Splitting a one-file script into a small test-framework project solved a problem nobody had (nicer structured output) at the cost of the one property that mattered (minimalism — a single file anyone, in any language ecosystem with Node available, can open and read top to bottom). **Reverted in full**: deleted `test/acceptance/`, restored `jest.config.js`/`package.json` to their pre-Phase-7 state.

**Lesson recorded for future changes to this layer:** when a design goal is explicitly "keep it as simple/portable as possible," resist the instinct to reach for this project's existing tooling (Jest, TypeScript) just because it's already there and "nicer" — that instinct is exactly what re-couples a deliberately-decoupled layer to this codebase's stack.

### Implementation (2026-07-15): username-casing regression added directly to `scripts/smoke-test.js`

Added the Phase 6 regression as one more check inside the existing single-file script, not a new file:
- Tracks which identifier (`userLogin` or `adminLogin`, whichever ends up backing `userToken`) was actually used, alongside the existing token-resolution logic.
- Runs a real create → update → delete cycle under that account, **only if its username contains an uppercase letter** — same honest constraint as before: this script can't register a brand-new mixed-case account (blocked by real email verification), so it can't fabricate the precondition on demand the way `test/e2e/finan.e2e.test.ts` does (which mints a locally-signed JWT for an arbitrary disposable username — fine there, but doing the same here would mean touching `jsonwebtoken`/the shared secret directly, breaking the "only ever speaks HTTP" property this script exists for).
- Skips with a clear, printed reason when the configured account is all-lowercase (true today: `anderokgo`) — stated plainly, not worked around.

**Verified for real** against the real Docker MariaDB + a running local server: **35/35 checks passed** (same total as before — the new check is additive but conditional, and correctly didn't run for the real all-lowercase account; confirmed its skip message prints clearly). No changes needed to `jest.config.js`/`package.json` this time — everything lives in the one file, exactly as before.

### Ongoing, not a TODO: growing the smoke test toward exhaustiveness

Expanding `scripts/smoke-test.js` coverage toward a genuinely *exhaustive* acceptance contract — every validation rule, every documented edge case per endpoint, not just the happy path — has no natural finish line the way the rest of this phase did, so it doesn't belong marked "TODO" under a phase that's otherwise closed. It's the same kind of open-ended, living work as the Acceptance Criteria Catalog itself (see that document's own "Status: living document" framing): every future bug hunt or module recovery sweep that surfaces a new non-obvious rule adds a catalog entry, and a smoke-test check when it's safe to automate one — Phase 8's `series` sweep (2026-07-18) is the most recent concrete instance of this pattern, closing 3 of the sweep's 10 findings this way (see that phase's table). **Within the existing single-file-script shape**, per the false-start lesson above, unless a future need for structured/CI-parseable output ever justifies revisiting that trade-off explicitly with the user first.

`docs/SPECIFICATION.md` (the other item originally listed here) shipped in Phase 8b — the black-box suite is the *behavioral* half of an executable spec; that document is the *architectural* half (why the code is shaped this way), which a language-agnostic HTTP suite deliberately has no opinion on.

---

## Phase 8 — `series` module business-rule recovery sweep

**Status: DONE** (2026-07-18)

Same method as the `finan` (Phase, 2026-07-15) and `auth` (2026-07-15) sweeps, applied to the one module that hadn't gotten it yet: read every source file in the module end to end (validation, all 9 command handlers, all 7 query handlers, both MySQL repositories, the image service, the controller, the module/route wiring) plus the relevant `animecream-data` SQL (views/procs/triggers), looking for comments, hardcoded constants, and special-case branching that a rewrite would silently get wrong.

Found 10 non-obvious things, 4 of them real bugs (not just undocumented behavior) — user reviewed all 4 and chose to fix every one, same pattern as the `expiresIn`/JWT-secret bugs found during the `auth` sweep:

| # | Finding | Disposition |
|---|---|---|
| 19 | `PUT /api/series/:id` ran the image-upload middleware but never read `req.file` — any image attached to a metadata update was silently parsed and discarded; only the separate `PUT /:id/image` endpoint worked | **Fixed** — `UpdateSeriesCommand`/`UpdateSeriesHandler` now process it the same non-fatal way `create` does |
| 20 | `updateSeries`'s controller used a truthy check (`req.body.chapter_number && ...`) to detect "field provided" — a real JSON `0` for `chapter_number`/`qualification` is falsy, so it was silently dropped (multipart form-data was unaffected, since `'0'` is a truthy string) | **Fixed** — switched to explicit `!== undefined && !== ''` checks |
| 21 | `findByNameAndYear` (the upsert-by-name+year duplicate check, catalog #5) didn't filter by `visible` — creating a series with the same name+year as a previously soft-deleted one revived it instead of creating a new row, silently undoing the soft-delete | **Fixed** — added `AND visible = 1` to the query |
| 22 | `update-series-image.handler.ts` validated a 10MB image-size limit that could never be reached — the shared `uploadMiddleware` (multer) already rejects anything over 5MB before the request reaches the handler | **Fixed** — handler's `MAX_SIZE` now matches multer's real 5MB |
| 23 | `view_all_info_produtions` (the SQL view behind the "boot" `POST /` endpoint) `INNER JOIN`s genres — a series with zero genres assigned is completely absent from that endpoint's results, despite being fully visible via `GET /:id`/`list`/`search` | Documented (catalog #23) — verified for real against the live DB, locked in as a permanent E2E test |
| 24 | `search`/`findAll`/`getProductions` all hardcode `visible = 1` with no override path — a soft-deleted series can never appear in any list/search result, not even for admins; `search()`'s own comment claims an admin override exists but it doesn't | Documented (catalog #24) |
| 25 | Series cover images are forced to exactly 190×285px, always re-encoded as JPEG regardless of upload format, compressed to ≤20KB | Documented (catalog #25) |
| 26 | Three listing endpoints (`list`/`search` vs. the legacy boot `POST /`) have three different, undocumented pagination defaults/caps (50/100 vs. 500/10,000) | Documented (catalog #26) |
| 27 | `remove-genres`/`remove-titles` don't validate ids exist (silent no-op on a bad id), unlike `assign-genres`; `add-titles` only dedupes within a single request, not against the series' existing titles (two calls with the same title create a real duplicate) | Documented (catalog #27) |
| 28 | `assignGenres`'s SQL uses raw string interpolation for genre ids — not exploitable today (every caller validates integers first) but the same shape as the real finan SQL-injection bug from Phase 5 | Documented (catalog #28) — hardening note, not fixed |

All 4 fixes verified for real, not just via mocked tests: `tsc --noEmit` clean, **1300/1300 tests / 86/86 suites / 100% coverage**, **31/31 E2E** (28 pre-existing + 3 new: the image-on-update regression, the genre-less-series-invisible-on-boot regression now locked in permanently, and the revive-prevention fix) against the real `animecream-mariadb` container, **52/52 smoke checks** (49 pre-existing + 3 new: image-on-update, and the genre-less-series absent-then-present pair) against a real locally-built server. Full findings and exact fix descriptions are in `docs/ACCEPTANCE_CRITERIA.md` entries #19-28.

Not yet committed — pending user review.

---

## Phase 8b — `docs/SPECIFICATION.md`: the generative design-rules layer

**Status: DONE** (2026-07-18)

Closes Phase 4's last remaining open item. Unlike `docs/architecture.md` (descriptive — "here's the current shape") or `docs/ACCEPTANCE_CRITERIA.md` (behavioral — "here's what must be true over HTTP"), this document is generative: the conventions an implementer (human or AI) needs to follow so new code *belongs* in this codebase, extracted by actually reading and cross-comparing all three modules' source rather than designed from a blank page.

Ten sections: layering direction, the fixed 3-directory module shape, the two coexisting business-logic patterns (CQRS in `series` vs. classic use-cases in `auth`/`finan` — a deliberate per-module choice, not migration debt), the composition-root shape, port placement rules (`application/ports/` for a module's own repository vs. `domain/ports/` for other external adapters), the two-pass validation convention (shape-check at the controller, business-rule-check inside the handler), a section on entities being pure data (no behavior-bearing domain classes anywhere in this codebase), multi-database/multi-repo boundaries, and testing conventions (the 100%-coverage mechanism that's what actually keeps the rest of this true).

The most consequential section (§7) is the one place the three modules genuinely disagree: `series`' CQRS handlers throw plain `Error`s that the controller catches and translates; `auth`/`finan`'s use-cases never throw for expected failures, instead resolving with a `{error/success, message, data}` envelope the controller mostly just forwards. Went further and grepped the actual JSON shapes returned across all three controllers rather than assuming uniformity: `series.controller.ts` alone uses **at least four different error-response shapes** across its 16 methods, and `finan.controller.ts`'s success responses have no `success`/`error` field at all (only `{message, data}`) while its failures do. Documented as real, verified inconsistency rather than smoothed over — unifying it would change every endpoint's wire contract at once, which needs an explicit decision first (same "flag, don't silently fix" rule that's governed every behavior-affecting change this session).

One genuine, if inert, finding surfaced while verifying the layering rule (§1) against real source rather than assuming it held: `finan`'s `application/ports/finan.repository.ts` imported `DataParams` from `infrastructure/models/dataparams.ts` — a real Clean Architecture boundary violation (`application` importing `infrastructure`), though the import was unused so it had zero runtime effect. Fixed directly (one-line removal, no behavior change, judged safe to just fix rather than ask given zero observable impact — unlike the four Phase 8 bugs, which all changed real behavior and were explicitly confirmed with the user first). A second, non-violating oddity was left as a documented-only "known deviation" rather than touched: `DataParams` itself lives in `infrastructure/models/` instead of `domain/entities/` (where every other request DTO in the codebase lives) — a safe rename, but out of scope for a docs-only pass.

Cross-referenced from `docs/README.md` and `docs/architecture.md`. Verified after the one code change: `tsc --noEmit` clean, **1300/1300 tests / 86/86 suites / 100% coverage** (unchanged from Phase 8 — the removed import was genuinely dead).

Not yet committed — pending user review.

---

## Phase 8c — Architecture Decision Records (`docs/adr/`)

**Status: DONE** (2026-07-18)

Closes the last open item in Phase 4 (item 4). An ADR answers *why* a major structural choice was made — different from `docs/architecture.md` (*what* the shape is today), `docs/SPECIFICATION.md` (*how* new code must be shaped to match), and `docs/ACCEPTANCE_CRITERIA.md` (*what* the system must do, observably). Most of the 8 ADRs written are retrospective — reconstructed from the code and this roadmap's own history rather than transcribed from a decision meeting that happened at the time — and each one says so explicitly rather than presenting invented context as if it were contemporaneous.

Eight ADRs, `docs/adr/0001` through `0008`: hexagonal/Clean Architecture layering (all modules), CQRS for `series` only (ADR-0002, cross-referencing `docs/SPECIFICATION.md` §3's reasoning), manual composition roots with no DI framework (confirmed via `grep` on `package.json` — no InversifyJS/tsyringe/Awilix dependency exists), one database per module with schema owned by the three sibling repos (ADR-0004), `finan`'s per-user dynamic tables (ADR-0005 — the one ADR grounded in an explicit, dated user decision: 2026-07-15, choosing to keep the per-user-table architecture over consolidating to a shared table, in direct response to the Phase 5 SQL-injection review that could have gone either way), soft-delete-for-`series`-vs-hard-delete-for-`finan` (ADR-0006, extending `docs/ACCEPTANCE_CRITERIA.md` #4 with the *why*, not just the *what*), `scripts/smoke-test.js` staying a single plain file (ADR-0007, built entirely from the real tried-and-reverted `test/acceptance/` episode in Phase 7 — a genuine alternative that was actually built and undone, not a hypothetical), and the 100%-coverage CI gate (ADR-0008, tracing directly back to the Phase 0 drift that motivated it and the Phase 4a coverage-artifact discovery that justified raising it from 80%).

Contract testing between modules (Pact or similar), the other half of the original Phase 4 item 4, stays explicitly **not** written as an ADR — no microservice split has happened (all three modules still run in one process behind one `server.ts`), so there is no real decision to document yet. Recorded as a "Deferred" note in `docs/adr/README.md` instead of a TODO with no trigger condition.

Cross-referenced from `docs/README.md` and `docs/architecture.md`. Documentation-only pass — no source changes, so no test/coverage re-verification needed beyond confirming the working tree was otherwise unchanged.

Not yet committed — pending user review.

---

## Phase 9 — Sibling-repo (`*-data`) schema-drift sweep and CI gates

**Status: DONE** (2026-07-19)

Closes the open item flagged in "Important: this is a multi-repo system" above: none of the three sibling schema repos (`auth-data`, `finan-data`, `animecream-data`) had ever received the kind of audit or CI gate `module-api` itself got in Phases 0–8c. Applied the same method as Phase 5's SQL audit (read every file end to end; diff the bootstrap against the real production schema captured in each repo's own `sql/db-data.sql`) plus Phase 3's CI-gate pattern, one repo at a time, each fix verified live against a disposable `mariadb:10.3.39` container before being committed.

### Findings and fixes

| Repo | # | Finding | Disposition |
|---|---|---|---|
| `auth-data` | 1 | `db-structure.sql` never got the 3 security columns (`login_attempts`, `last_login`, `locked_until`) added by `sql/migrations/0001_add_user_security_fields.sql` — a standalone bootstrap silently produced a `users` table missing the columns the account-lockout feature (catalog #13) depends on | **Fixed** |
| `auth-data` | 2 | `email_verification` bootstrapped with the server-default engine/charset instead of production's real `ENGINE=MyISAM CHARSET=latin1 COLLATE=latin1_swedish_ci` | **Fixed** |
| `auth-data` | 3 | `db-trigger.sql` had a stale copy-pasted comment (`` Triggers `productions` ``) — `animecream-data`'s table, not this repo's | **Fixed** |
| `auth-data` | 4 | Migration `0001`'s comments were in Spanish, violating this repo's own `CLAUDE.md` English-only convention | **Fixed** |
| `auth-data` | 5 | Initially misdiagnosed as a git-history PII leak (`sql/db-data.sql`, a real production dump of emails + bcrypt hashes, appeared to have been tracked before it was gitignored). Re-checked by actually reading each historical commit's content (`git show <sha>:sql/db-data.sql`), not just its tracked-file history — every one only ever held the symlink's target-path string, never the resolved data. **Not a real finding**; the original write-up (here and in `auth-data/README.md`) was corrected the same session | No action needed — corrected, not fixed (nothing was ever wrong) |
| `finan-data` | 6 | `constants`/`type_sources` bootstrapped as `MyISAM/utf8mb4/utf8mb4_unicode_ci`; real production is `InnoDB/utf8/utf8_general_ci`, and `constants` was also missing the `idx_currency_id` index that exists in production | **Fixed** |
| `finan-data` | 7 | Same stale `` Triggers `productions` `` comment as `auth-data` | **Fixed** |
| `finan-data` | 8 | `proc_view_yearly_expenses_incomes` and `proc_view_monthly_expenses_incomes_order_row` subtract a hardcoded `5380` from 2022 income rows (`type_source_id IN (1, 13)`); `view_monthly_expenses_incomes` and `proc_view_total_balance` don't — the same underlying rows produce two different 2022 totals depending which endpoint is hit | **Documented only** (inline `FLAG` comments at both call sites in `db-views-procs.sql`) — origin and continued validity unconfirmed, needs a product decision before being touched either way |
| `finan-data` | 9 | Re-verified Phase 5's SQL-injection fix (bind values via `EXECUTE ... USING`, username regex guard) is still intact across all 9 procedures | No action needed |
| `animecream-data` | 10 | `sql/migrations/README.md`'s rule ("migrations never touch data or run `DROP TABLE`") is contradicted by this repo's own `0001_drop_users_table.sql` | **Fixed** — the rule now documents the one confirmed, deliberate exception instead of being silently contradicted by it |
| `animecream-data` | 11 | `db-trigger.sql` was missing the trailing `SET FOREIGN_KEY_CHECKS = 1;` that both sibling repos have as their last line | **Fixed** |
| `animecream-data` | 12 | `db-structure.sql` created the database but never ran `USE animecre_cake514` — a standalone bootstrap (`mysql -u... -p... < db-structure.sql`, no DB name on the CLI, exactly what the README says this file is for) failed with "No database selected" on the first `DROP TABLE`. Found live, during verification of findings #10/#11, not by static reading | **Fixed** |

All three repos' `sql/db-data.sql` git history was checked the same way, by reading actual historical commit content rather than just tracked-file history: all three were always just the symlink-path placeholder string, in every historical commit — clean across the board, nothing to document (see #5's correction note above for how this was initially gotten wrong for `auth-data` specifically).

### Verification method

Each fix was verified against a disposable `mariadb:10.3.39` container (same version as `module-api/docker/Dockerfile`, i.e. production's real version) — never the long-running `animecream-mariadb` dev container, and never reasoned about statically. For `auth-data`/`finan-data`: applied the fixed `db-structure.sql`, then `SHOW CREATE TABLE` on the affected tables, diffed column-for-column against the real production structure captured in each repo's own `sql/db-data.sql`. For `animecream-data`: applied `db-structure.sql` → `db-views-procs.sql` → `db-trigger.sql` standalone, confirmed all 5 tables + 7 views + `update_rank()` match production, and smoke-called `update_rank()` across its 0-row/1-row/N-row branches (inserting real rows into `demographics`/`productions` inside the disposable container) to confirm the Phase 5 cursor → set-based rewrite still produces correct ranks. Every throwaway container was removed immediately after (`docker rm -f`); the persistent `animecream-mariadb` dev container was never touched.

Committed and pushed per-repo: `auth-data@f6dae9f`, `finan-data@77d7007`, `animecream-data@90ca1f9`.

### CI gates

None of the three sibling repos had any CI at all before this pass (confirmed: no `.github/workflows` directory in any of them) — the only thing that existed was the manual, confirmation-gated deploy pipeline documented in each repo's own `docs/deployment.md`, which is not the same thing: nothing ran automatically on push/PR to catch drift like findings #1/#6/#12 above before a human happened to check by hand.

Added `.github/workflows/ci.yml` to all three, same shape in each: a real `mariadb:10.3.39` service container, `db-structure.sql` applied standalone (no DB name on the CLI, so the same class of bug as #12 can't hide again) → `db-views-procs.sql` → `db-trigger.sql`. The core mechanism is a **drift detector**: snapshot the table structure (`mysqldump --no-data`) right after the bootstrap, apply every `sql/migrations/*.sql` file on top, snapshot again, fail the build on any diff — `db-structure.sql` is supposed to already reflect every migration, so re-applying them should be a pure no-op. This automates exactly the manual check that found #1 and #6, so that class of drift can't silently reappear.

Two repos got an extra, repo-specific smoke step on top of the shared shape:
- `finan-data`: calls all 9 stored procedures against a disposable `ci_smoke_test` table (catches dynamic-SQL syntax regressions), and asserts `proc_create_movements_table` still rejects a malicious username with `Invalid username` rather than succeeding — a live regression test for Phase 5's SQL-injection fix.
- `animecream-data`: calls `update_rank()` across its 0-row/1-row/N-row branches, the same three cases Phase 5's cursor → set-based rewrite had to preserve.

Every workflow was simulated locally against a disposable container (identical commands to what the YAML runs) before being committed, then independently confirmed green by querying the real GitHub Actions API after push — not assumed from a local pass. All three runs: `success`, ~37–41s each (`auth-data` run `29689586569`, `finan-data` run `29689587324`, `animecream-data` run `29689588350`). `module-api`'s own pre-existing CI gate (Phase 3) was re-checked at the same time and confirmed still green against current `HEAD` (run `29661405142`, 34s, type-check + 100%-coverage test gate).

Committed and pushed per-repo: `auth-data@cbdde67`, `finan-data@0d5b8eb`, `animecream-data@c3f4bc3`.

### Open question, not decided: one schema repo per module vs. a monorepo

Asked directly (2026-07-19) whether the 3-repo split was good architecture, separately from whether the docs describing it were accurate. Answered with a recommendation, not a decision: the *logical* separation (each module owns its own schema) is good practice and matches the hexagonal/CQRS module boundaries `module-api` already has; the *physical* separation into 3 full git repos looks more questionable at this project's actual scale — the `com/*.bat` deploy-script folder is byte-for-byte duplicated across all three (per `docs/deployment.md`: "identical across all three repos... copies of the same template"), so any infrastructure fix (the `DEFINER=` bug, for one) has to be manually reapplied three times; no microservice split has actually happened (all three modules still run in one process behind `module-api`'s one `server.ts`, per ADR-0008's own framing), so the strongest justification for separate repos doesn't fully apply yet; and this session's own findings are direct evidence of the coordination cost — 2 of the 3 repos had real, undetected schema drift from production. Suggested alternative (not built): a monorepo with per-module schema subfolders, or at minimum de-duplicating the shared script template. **Left for the user to decide** — not implemented, not scheduled.

---

## Acceptance Criteria Catalog

**Moved to `docs/ACCEPTANCE_CRITERIA.md`** (2026-07-18) — split out once it reached 18 entries, so this evergreen, always-current reference doesn't keep growing inside this file's day-by-day project log. The methodology (real-rule vs. implementation-robustness, the catalog/script/source three-way relationship, how it grows) and all 18 entries live there now. This file remains the place for *how* each entry was discovered — see the Progress log below for that history.
...
- **2026-07-15** — User raised the real backlog problem directly: this project has been going for a while, and plenty of business rules already became code (some only explained in comments, most not explained at all) before this catalog existed — how do we recover that? Agreed on a targeted, module-by-module reading sweep (comments, validation functions, hardcoded special-cases/constants) rather than trying to grep for keywords (tried a quick keyword scan first — only 5 hits out of 548 comment lines, confirming grep alone can't find this reliably). Started with `finan` per the user's choice. Found 6 real, previously-undocumented rules, the most concrete one confirmed against live production data: the real `type_sources` catalog has 9 categories, but the app's `MovementType` enum + validation only allow 3 of them — `movements_anderokgo` genuinely has 8 rows of type 13 ("cash exchange") and 3 of type 8 ("balance") that could never be *created* through today's API. Also found: a single hardcoded username (`anderokgo`) unlocks extra response data; the "general/trip info" endpoints it unlocks are a hardcoded personal GYG/travel-fund tracker, not a general feature; the monthly budget calculation embeds a specific personal financial formula (savings goal, health/pension deduction, payroll fallback); `operate_for`/the `log` column implement a one-way linked-movement value adjustment with no undo; and finan's duplicate-prevention (same name+date) returns the original record unmodified, unlike series' upsert-by-name+year which does update. Added all 6 as catalog entries #6-11. Added real `scripts/smoke-test.js` checks for the 3 that are safe/meaningful to automate with disposable fixtures (#6 type-validation gap, #10 linked-movement adjustment, #11 duplicate-returns-original) — verified **48/48 passed**, cleanup confirmed via direct DB query. Left #7/#8/#9 documented-only, with the reasoning recorded in each entry (tied to one real person's actual identity/financial data, not safely automatable). Not yet committed.
- **2026-07-15** — Continued the recovery sweep with `auth`. Found 5 more rules (catalog entries #12-16), the standout being a real bug, not just an undocumented rule: the JWT's real lifetime is 30 days, but the login response's `expiresIn` field hardcoded `86400` (24 hours) — completely disconnected from the actual token. Asked the user how to handle it; they chose to fix `expiresIn` to reflect the real 30 days. Fixed by adding `getExpiresInSeconds()` to `TokenGeneratorPort`/`JwtTokenGeneratorService`, deriving both the JWT string and the seconds value from one constant so they can't drift apart again; `login.use-case.ts` now reads it instead of hardcoding. Also found: account lockout fires on the 5th consecutive failed attempt (not the 4th, despite the `>= 4` check reading that way), locks for exactly 15 minutes, clears only on the next successful login; login resolves username-or-email with username taking priority; registration can only ever create `role: USER` accounts (no API path to admin); and `UserRepository.update/delete/unlockUser` are fully implemented but never called by anything. Mid-sweep, the user asked for a genuinely new feature tied directly to that last finding: an admin password-reset endpoint, so real test accounts (including future mixed-case ones) can be managed without DB access. Asked for a design decision (self-service vs. admin-reset vs. both); user chose admin-reset only. Implemented `AdminResetPasswordUseCase`, wired `PUT /api/users/admin/reset-password` behind `validateAdmin`, with full unit + integration + real E2E coverage (catalog entry #17). Verified end to end: `tsc --noEmit` clean, **1290/1290 tests / 85/85 suites / 100% coverage**, **28/28 E2E** (26 pre-existing + 2 new for the reset endpoint), **49/49 smoke checks** (48 pre-existing + 1 new read-only regression guard for the `expiresIn` fix — the reset endpoint itself deliberately excluded from the smoke test, since automating it would mean changing a real account's real password on every run). Not yet committed.
- **2026-07-18** — User asked whether the code itself (not just the docs' claims about it) was correct. Verified the previously-documented state for real first (`tsc --noEmit` clean, **1290/1290 tests / 85/85 suites / 100% coverage**, matching the doc exactly, `HEAD` at `bafce18` with a clean working tree — confirming the last progress-log entry's "Not yet committed" note was itself just stale). Then went further and read core source directly rather than trusting the roadmap's own self-assessment: `runInTransaction`'s rollback/release handling, `assignGenres`'s real transaction usage, the hexagonal domain-layer import boundary (zero `infrastructure/` imports found), and the auth flow. Found a real bug the catalog had never caught: three call sites each had their own hardcoded JWT-secret fallback, and the fallbacks disagreed with each other (signing fell back to `'enterkey'`, verification to `'qwertgfdsa'`) — see catalog entry #18 for the fix. User chose fail-fast-at-startup over the smaller "just make the fallbacks consistent" alternative. Fixed and verified for real (compiled build, run with/without `SECRET_KEY` in a directory with no `.env`, confirmed the crash-before-DB-connection and the normal-startup paths directly, not just via mocked tests). **1293/1293 tests / 86/86 suites / 100% coverage.** Not yet committed.
- **2026-07-18** — User pointed out doc fragmentation: `docs/architecture.md`, `docs/architecture-analysis.md`, `docs/architecture-final-report.md` all covered the same ground (the latter two were Oct-2025 before/after snapshots of a Clean Architecture refactor, long since superseded), and each module had a duplicate pair (`docs/refactoring-{module}-summary.md` + `docs/modules/{module}.md`). Asked to combine into one file per topic and update whatever was stale. Found the module docs were badly out of date — `modules/auth.md`/`modules/finan.md` described directory structures and features (a `categories` table with colors, 8-char+complexity password rules, 30-minute lockout, 12 bcrypt rounds) that never matched the real code, not just prose that had drifted. Rewrote all of it against actual source: `architecture.md` (general shape + a condensed history paragraph replacing the two superseded docs), and `modules/{auth,finan,series}.md` (real structure, real endpoints, pointers to the relevant catalog entries instead of re-describing the rules inline). Deleted the 5 superseded files, fixed `docs/README.md`'s links (it pointed at a `troubleshooting.md` and `docs/postman/` that never existed) and its wrong/duplicated database list. Not yet committed.
- **2026-07-18** — User asked, more broadly, for the most efficient way to keep accumulating both business logic *and* the "experience" (bugs found, decisions made, why) without fragmenting into many files, given the end goal of a from-scratch rebuild staying 100% behaviorally equivalent. Answered: the Catalog + phase/progress-log combo already does this, but the single `specification-roadmap.md` file was starting to mix two things with very different lifespans — the evergreen Catalog (should stay short and always-current) and the day-by-day phase/progress log (historical, append-only, fine to let grow). Recommended splitting by that distinction, not by module or topic, capping it at 2 files total. User agreed. **Done**: moved the full Acceptance Criteria Catalog (methodology + all 18 entries) to a new standalone `docs/ACCEPTANCE_CRITERIA.md`; this file now just points to it and keeps the Phases/Progress log as the historical record. Updated cross-references in `docs/README.md`, `docs/architecture.md`, `docs/modules/*.md`, and one path-specific comment in `scripts/smoke-test.js`. Not yet committed.
- **2026-07-18** — User asked to continue the initiative; agreed the `series` module business-rule recovery sweep (the one module `finan`/`auth` had already gotten, per Phase 4's remaining TODO items) was the best-scoped next step. Read the entire module plus the relevant `animecream-data` SQL and found 10 things, 4 of them real bugs — presented all 4 to the user via explicit questions rather than assuming a fix vs. document-only call, matching the established pattern from the `auth` sweep's `expiresIn` bug. User chose to fix all 4. Implemented the fixes, added regression tests at every layer (unit, one new E2E, smoke test), and documented all 10 findings as catalog entries #19-28. Verified for real: Docker was down at the start of the session and was found already running mid-task; used it to empirically confirm the genre-less-series-invisible-on-boot finding (a real INNER JOIN, not just reasoned about) before writing it into the catalog. Full gate: `tsc --noEmit` clean, **1300/1300 tests / 86/86 suites / 100% coverage**, **31/31 E2E**, **52/52 smoke checks**, all against the real database and a real locally-built server. See Phase 8 above for the full findings table. Committed as `module-api@59b066e` and pushed to `origin/main` at the user's request.
- **2026-07-18** — User asked what else was left on the roadmap; recommended `docs/SPECIFICATION.md` (Phase 4's last unstarted item) as the best next step, since it's the one item with no scope ambiguity and directly complements the catalog work just finished. User agreed. Read and cross-compared all three modules' actual source (composition roots, port placement, entity shapes, controller error-handling) rather than designing from the existing descriptive docs, and wrote the 10-section generative spec described in Phase 8b above — including grepping the real HTTP response shapes across all three controllers, which turned up more inconsistency (four different error shapes inside `series.controller.ts` alone) than expected. Found and fixed one inert layering violation (`finan.repository.ts` importing, unused, from `infrastructure/`) along the way; left a second, non-violating structural oddity (`DataParams` living outside `domain/entities/`) as a documented-only deviation. Verified: `tsc --noEmit` clean, **1300/1300 tests / 86/86 suites / 100% coverage**, unchanged from before the one code fix. Committed as `module-api@3b457dd` and pushed to `origin/main` at the user's request.
- **2026-07-18** — User asked to continue with the ADRs (Phase 4 item 4, the last unclosed item once `SPECIFICATION.md` landed). Wrote `docs/adr/` — an index plus 8 ADRs covering the real structural decisions already made across this project's life, grounded in the roadmap's own history rather than invented: hexagonal layering, CQRS-for-`series`-only, manual DI, per-module databases, `finan`'s per-user tables (the one ADR tied to an explicit, dated user decision — 2026-07-15), soft-vs-hard delete, the smoke-test single-file rule (built from the real `test/acceptance/` episode that was tried and reverted in Phase 7), and the 100%-coverage CI gate. Left contract-testing between modules (the other half of the original TODO item) explicitly undocumented as an ADR, since no microservice split has happened — nothing to write a decision about yet. Documentation-only pass, no source changes. See Phase 8c above.
- **2026-07-18** — User asked what was left on the roadmap; every phase except Phase 7 was already `DONE`. Phase 7's own "Still TODO" list turned out to be stale (item 2, `docs/SPECIFICATION.md`, had already shipped in Phase 8b) and item 1 (exhaustive smoke-test coverage) was never actually a task with a finish line — it's the same kind of living, ongoing work as the Acceptance Criteria Catalog itself. User agreed to close the phase on that basis. Reframed Phase 7's status to `DONE` and rewrote its "Still TODO" section as "Ongoing, not a TODO," pointing at the Catalog's own living-document framing and at Phase 8 as the most recent concrete instance of the pattern (3 of its 10 findings became new smoke-test checks). Documentation-only, no source changes.
- **2026-07-19** — Asked what from `module-api`'s own executable-spec work could apply to the 3 sibling `*-data` repos, which had never gotten any of it. Surveyed all three (confirmed: no `.github/workflows` in any of them) and recommended a targeted sweep per repo plus CI gates, mirroring Phase 5's SQL audit and Phase 3's CI-gate pattern. User agreed to start with `auth-data`.
- **2026-07-19** — Swept `auth-data`: read `db-structure.sql`/`db-views-procs.sql`/`db-trigger.sql`/`sql/migrations/`, and diffed the bootstrap against real production structure captured in the repo's own (gitignored, symlinked) `sql/db-data.sql`. Found 5 things (see Phase 9's table); user chose to fix 4 and document-only the 5th (an apparent git-history PII leak). Verified the fix for real against a disposable `mariadb:10.3.39` container (Docker wasn't running at first — started it, then used it), not just reasoned about: the fixed bootstrap now matches production column-for-column. Committed `auth-data@f6dae9f` and pushed to `origin/main` at the user's request.
- **2026-07-19** — Swept `finan-data` the same way. Found 4 things: 2 real engine/charset drift bugs (fixed, verified live), a stale copy-pasted trigger comment (fixed), and an undocumented, inconsistently-applied `-5380` correction for 2022 income rows present in 2 of the 4 aggregation paths (flagged with inline `FLAG` comments, left for a product decision rather than guessed at). Also confirmed Phase 5's SQL-injection fix is still intact across all 9 procedures. Committed `finan-data@77d7007` and pushed to `origin/master`.
- **2026-07-19** — Swept `animecream-data`, which turned out much cleaner already (Phase 5/8 had touched it directly). Found 2 static issues (a migration-vs-documented-rule contradiction, a missing FK-checks reset) plus one found only by testing live: `db-structure.sql` never selected the database it created, so a standalone bootstrap — exactly what the README says this file is for — failed with "No database selected." Fixed all 3, verified end to end against a disposable container including `update_rank()` smoke-called across its 0-row/1-row/N-row branches. Committed `animecream-data@90ca1f9` and pushed to `origin/main`.
- **2026-07-19** — User asked two exploratory questions: whether the docs across the 3 sibling repos are accurate/complete (including how the manual deploy pipeline and migrations work), and whether splitting the database into 3 repos from `module-api` is good architecture. Answered directly rather than deferring: the deployment-mechanics docs (`docs/deployment.md`) are strong, but the schema-truth docs had real, just-fixed drift, and — importantly — there is no actual CI in any of the 3 repos, only a well-documented *manual* deploy pipeline, which is a different thing. On the architecture question: recommended keeping the *logical* per-module schema separation but questioned the *physical* 3-repo split at this project's real scale, citing the byte-for-byte duplicated `com/*.bat` template (any infra fix has to be hand-applied 3 times), the fact that no microservice split has actually happened yet, and this session's own drift findings as evidence of the coordination cost. Suggested a monorepo alternative, not built. **Left open, not decided.**
- **2026-07-19** — User asked to build the missing CI gate. Added `.github/workflows/ci.yml` to all 3 sibling repos: a real `mariadb:10.3.39` service container, bootstrap applied standalone, then a **drift detector** (snapshot table structure, apply all migrations on top, snapshot again, fail on any diff) that automates the exact manual check that found the drift bugs above. `finan-data` additionally smoke-calls all 9 procedures and regression-tests the SQL-injection guard; `animecream-data` additionally smoke-tests `update_rank()`'s 3 branches. Every workflow was simulated locally against a disposable container before being committed. Committed and pushed: `auth-data@cbdde67`, `finan-data@0d5b8eb`, `animecream-data@c3f4bc3`.
- **2026-07-19** — Checked the real GitHub Actions runs for all 3 (via the API, using the credential-helper's cached token — no `gh` CLI installed in this environment) rather than assuming green from the local simulation. All 3 came back `success`: `auth-data` run `29689586569` (37s), `finan-data` run `29689587324` (41s, all 13 steps including the smoke-calls and the injection-guard check green), `animecream-data` run `29689588350` (39s, including the 3-branch `update_rank()` smoke test). Also re-checked `module-api`'s own pre-existing CI gate at the same time, purely as a sanity check — still green against current `HEAD` (run `29661405142`, 34s). Documented all of the above as Phase 9.
- **2026-07-19** — While explaining *why* the `keys/`/Google-Drive-symlink credential scheme is fragile, cited `auth-data` finding #5 (a supposed git-history PII leak) as live evidence. Re-verifying it properly this time (`git show <sha>:sql/db-data.sql` against every historical commit, the same check already correctly done for `finan-data`/`animecream-data` back when #5 was first written) showed it was wrong: `auth-data`'s committed content was, in every commit, only ever the symlink's target-path string — identical to the other two repos, never real data. The original Phase 9 finding and the note it produced in `auth-data/README.md` were both written from reading the *current, locally-resolved* symlink target (which does hold real data on disk today) without separately checking what git itself had actually stored — an unverified inference presented as a verified finding. Corrected both documents in place (this file's Phase 9 table and `auth-data/README.md`) rather than leaving the wrong claim standing; the credential-scheme critique itself doesn't depend on this specific incident and stands on the independently-documented Windows symlink-privilege and `DEL`-vs-symlink issues in `docs/deployment.md`.
