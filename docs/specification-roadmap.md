...
**Verified in `scripts/smoke-test.js`:** yes (2026-07-15, added alongside this entry) — asserts a create with `movement_type: 13` is rejected (400), confirming the limitation is real and intentional-by-inaction, not a fluke.
...
**Verified in `scripts/smoke-test.js`:** yes (2026-07-15, added alongside this entry) — creates a movement, then a second one with `operate_for` pointing at the first and a known value, and asserts the first movement's value changed by exactly the expected amount.

#### 11. Re-submitting the same movement (same name + date) returns the *original* untouched — it does not update, unlike series

**Rule:** `POST /api/finan/insert` checks for an existing movement with the same `name` + `date_movement` (per user) before inserting; if one exists, it returns success with the *existing, unmodified* record — even if the new request's value, type, tag, or currency differ from what's stored. This exists to make offline-sync retries idempotent. It is a **materially different upsert semantic from series' create** (Catalog entry #5), which *does* update the existing row's fields on a name+year collision — a rewrite that made both endpoints behave the same way (either both update or both no-op) would be wrong for one of them.
...
**Verified in `scripts/smoke-test.js`:** yes — the real-login check now also asserts `expiresIn` equals `30 * 24 * 60 * 60`, a read-only regression guard against this exact drift recurring. Unit-level: `jwt-token-generator.service.test.ts` cross-checks the claim against the token's own decoded `exp`/`iat`, not just the constant.
...
**Verified in `scripts/smoke-test.js`:** not automated — would need two distinct real accounts deliberately mismatched to observe, not safely constructible from disposable fixtures (registration can't be completed via this script - see the existing registration-limitation note).
...
**Verified in `scripts/smoke-test.js`:** not automated — would require completing a real registration (blocked by email verification, the same standing limitation noted elsewhere) to observe the created role.
...
Added this session, at the user's request, directly motivated by entry #16: a way to actually reset a real account's password (no current password required), gated by `validateAdmin` (real admin JWT required). Built specifically so the project owner can rotate/fix the password on existing test accounts (e.g. to set up a mixed-case-username account for entry #3-style regression testing) without needing direct database access.

**Enforced in:** `admin-reset-password.use-case.ts` (new), wired in `user.controller.ts`/`auth.module.ts` behind `validateAdmin`. Also clears any existing lockout (`resetLoginAttempts()`) as part of the reset, so a freshly-reset account is immediately usable.
...
Also added `jest.setup.js` (`setupFiles` in `jest.config.js`) so the test suite always has *some* `SECRET_KEY` value before any test file's modules load, since the new startup guard would otherwise fire for any test that transitively imports `server.ts`/the auth middleware without itself caring about auth. **1293/1293 tests passing, 86/86 suites, 100% coverage** (up from 1290/85 — one new test file for the guard itself, plus 1-for-1 replacements of the three tests that had locked in the old fallback behavior as "correct").
