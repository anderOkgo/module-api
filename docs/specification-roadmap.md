...
| `auth-data` | 5 | `sql/db-data.sql` (a real production dump: emails + bcrypt hashes) was a tracked file in early commits, before it was gitignored — still recoverable from `origin/main`'s history | **Documented only** (README.md), not purged — rewriting shared git history is a separate, deliberate decision the user chose not to fold into this pass. Repo confirmed private (an anonymous fetch returns 404), which limits the exposure |
...
All three repos' `sql/db-data.sql` git history was checked the same way: only `auth-data`'s ever held real tracked data (#5 above); `finan-data`'s and `animecream-data`'s were always just the symlink-path placeholder string, in every historical commit — clean, nothing to document there.
...
- **2026-07-19** — Swept `auth-data`: read `db-structure.sql`/`db-views-procs.sql`/`db-trigger.sql`/`sql/migrations/`, and diffed the bootstrap against real production structure captured in the repo's own (gitignored, symlinked) `sql/db-data.sql`. Found 5 things (see Phase 9's table); user chose to fix 4 and document-only the 5th (real PII/bcrypt hashes in early git history, repo confirmed private). Verified the fix for real against a disposable `mariadb:10.3.39` container (Docker wasn't running at first — started it, then used it), not just reasoned about: the fixed bootstrap now matches production column-for-column. Committed `auth-data@f6dae9f` and pushed to `origin/main` at the user's request.
- **2026-07-19** — Swept `finan-data` the same way. Found 4 things: 2 real engine/charset drift bugs (fixed, verified live), a stale copy-pasted trigger comment (fixed), and an undocumented, inconsistently-applied `-5380` correction for 2022 income rows present in 2 of the 4 aggregation paths (flagged with inline `FLAG` comments, left for a product decision rather than guessed at). Also confirmed clean: no PII was ever committed to this repo's git history (unlike `auth-data`), and Phase 5's SQL-injection fix is still intact across all 9 procedures. Committed `finan-data@77d7007` and pushed to `origin/master`.
