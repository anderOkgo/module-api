...
export const addUsers = async (req: Request, res: Response) => {
  ...
  if (resp.errorSys) res.status(500).json(resp.message);
  resp.error ? res.status(404).json(resp.message) : res.status(200).json(resp.message);
};
...
export const loginUsers = async (req: Request, res: Response) => {
  ...
  if (resp.errorSys) res.status(500).json(resp.message);
  resp.error ? res.status(404).json(resp.message) : res.status(200).json({ token: resp.token });
};
